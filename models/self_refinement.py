...
class SelfRefinementEngine:
    ...
    def load_logic_programs(self):
        ...
        with open(
            os.path.join(
                ...
                f"{prefix}{self.dataset_name}_{self.split}_{self.model_name}.json",
            )
        ) as f:
            dataset = json.load(f)
    ...
    def single_round_self_refinement(self):
        ...
        for example in tqdm(self.logic_programs):
            ...
            answer, status, error_message = self.safe_execute_program(
                example["id"], logic_program
            )

            if status == "execution error":
                if (
                    not error_message == "No Output"
