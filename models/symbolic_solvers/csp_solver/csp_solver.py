...
class CSP_Program:
    ...
    def safe_execute(self, code_string: str, keys=None, debug_mode=False):
        ...
        try:
            ans, error_msg = func_timeout.func_timeout(
                self.timeout, execute, args=(code_string,)
            )
    ...
    def parse_numeric_constraint(self, constraint):
        ...
        str_variables_in_rule_with_quotes = (
            ", ".join([f'"{v}"' for v in unique_list]) + ","
        )
    ...
    def execute_program(self, debug_mode=False):
        ...
        for variable in self.Variables:
            ...
            variable_name, variable_domain = (
                variable_name.strip(),
                variable_domain.strip(),
            )
            # variable_domain = ast.literal_eval(variable_domain)
            python_program_list.append(
                f'problem.addVariable("{variable_name}", {variable_domain})'
            )
    ...
    def answer_mapping(self, answer):
        ...
        for option_str in self.Query:
            ...
            if (
                len(variable_ans_map[variable]) == 1
                and value in variable_ans_map[variable]
            ):
                return option
